...
//    private Application application;
    ...
    private static HashSet<String> scriptCallStack = new HashSet<>();

    /* Contains all commands' description */
    private final static LinkedHashMap<String, String> descriptionsMap;

    private static LinkedList<String> commandsWithMovieArg = new LinkedList<>();
...
//            CommandsFactory.registerCommand("save", SaveCommand.class);
    ...
    public ConsoleManager(IOManager ioManager) {
        ...
        clientFileManager = new ClientFileManager(ioManager);
    }
     ...
     * 
     * @return  request object if request was created, otherwise null
     */
    public Request execute() throws FactoryException {
        String input;
        LinkedList<String> splitInput = new LinkedList<>();
        String commandTag;

//        if (readFromFile) {
//            if (ioManager.hasNext()) {
//                input = ioManager.getNextInput();
//            } else {
//                if (ioManager.popScanner()) {
//                    readFromFile = false;
//                }
//                return null;
//            }
        ...
        commandTag = splitInput.pollFirst();
        ...
        switch (commandTag) {
            ...
            case ("execute_script"):
                ...
                String scriptName = splitInput.get(1);
                try {
                    executeScript(scriptName, scriptCallStack);
            ...
            default:
                ...
                if (commandsWithMovieArg.contains(commandTag)) {
                    ...
                    commandArgs = new Object[] {movie};
                } else {
                    commandArgs = new String[splitInput.size()];
                    for (String arg : splitInput) {
                        commandArgs[splitInput.indexOf(arg)] = arg.trim();
                    }
                }
...
//     * @return                      "true" if program must terminate after executing script (if there was "exit" command in the script)
     ...
     */
    public void executeScript(String fileName, HashSet<String> stackTrace) throws FileException, IOException, RecursionException {
        if (stackTrace.contains(fileName)) {
            throw new RecursionException("Recursion has been found");
        }
        File file = clientFileManager.openFile(fileName);
